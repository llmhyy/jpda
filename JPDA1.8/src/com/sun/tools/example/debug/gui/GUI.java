	...
	public static void main(String argv[]) {
		...
		for (int i = 0; i < argv.length; i++) {
			...
			token.equals("-v") || token.startsWith("-v:") || // -v[:...]
					...
					// accepts them)
			token.equals("-noasyncgc") || token.equals("-prof") || token.equals("-verify") || token.equals("-noverify")
					|| token.equals("-verifyremote") || token.equals("-verbosegc") || token.startsWith("-ms")
					|| token.startsWith("-mx") || token.startsWith("-ss") || token.startsWith("-oss")) {
				javaArgs += token + " ";
